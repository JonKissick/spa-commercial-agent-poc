...
import { seedRagStarterPack } from "@/lib/api";
...
export default function RagLibraryPanel({ knowledge, isLoading, error, onRefresh }: RagLibraryPanelProps) {
  ...
  async function handleSeedPack() {
    setIsSeeding(true);
    setSeedSummary(null);
    setSeedError(null);
    try {
      const result = await seedRagStarterPack();
      setSeedSummary(`Starter pack: ${result.entries_ingested} ingested, ${result.duplicates_skipped} duplicates skipped.`);
      await onRefresh();
    } catch (err) {
      setSeedError(err instanceof Error ? err.message : "Failed to load starter knowledge pack.");
    } finally {
      setIsSeeding(false);
    }
  }
